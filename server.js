...
app.get("/api/zunzuns", (req, res) => {
  const sortedZunzuns = [...zunzuns].sort(
...
app.post("/api/zunzuns", isAuthenticated, (req, res) => {
  ...
  if (!text || text.length > 280) {
    return res
      ...
      .json({
        success: false,
        message: "Texto do zunzum inválido (máx 280 caracteres).",
      });
  }
  ...
  console.log(
    `Novo Zunzum de ${newZunzum.author} (Anônimo: ${newZunzum.isAnonymous}): ${newZunzum.text}`
  );
